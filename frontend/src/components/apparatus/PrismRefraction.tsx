import { useEffect, useRef, useState } from 'react';
import { useLabStore } from '../../store/useLabStore';
import gsap from 'gsap';

interface ApparatusProps {
  varState: Record<string, number | string>;
  setVarState: React.Dispatch<React.SetStateAction<Record<string, number | string>>>;
  addObservation: (data: any) => void;
}

function useWindowWidth() {
  const [width, setWidth] = useState(
    typeof window !== 'undefined' ? window.innerWidth : 1280
  );
  useEffect(() => {
    const handler = () => setWidth(window.innerWidth);
    window.addEventListener('resize', handler);
    return () => window.removeEventListener('resize', handler);
  }, []);
  return width;
}

export default function PrismRefraction({ varState, addObservation }: ApparatusProps) {
  // ── Read all variables from varState ──────────────────────────────────
  const incidentAngle = Number(varState.angle       ?? varState.i        ?? 45);
  const materialIdx   = Number(varState.material    ?? varState.n        ?? 1.5);
  // Prism apex angle — support every likely key the experiments data might use
  const A = Number(
    varState.prismAngle  ?? varState.apexAngle  ?? varState.prism_angle ??
    varState.apex        ?? varState.a          ?? varState.A           ?? 60
  );

  const { setValidationError, hasAdjustedSlider } = useLabStore();
  const windowWidth = useWindowWidth();
  const isMobile    = windowWidth < 768;

  // ── Physics ───────────────────────────────────────────────────────────
  const A_rad  = (A * Math.PI) / 180;
  const i1_rad = (incidentAngle * Math.PI) / 180;

  const sinR1  = Math.sin(i1_rad) / materialIdx;
  const r1_rad = sinR1 >= 1 ? Math.PI / 2 : Math.asin(sinR1);
  const r2_rad = A_rad - r1_rad;

  let e_rad = 0;
  let TIR   = false;
  const sin_e = materialIdx * Math.sin(r2_rad);
  if (sin_e > 1 || r2_rad < 0) {
    TIR = true;
  } else {
    e_rad = Math.asin(sin_e);
  }

  const emergentAngle = (e_rad * 180) / Math.PI;
  const deviation     = TIR ? 0 : incidentAngle + emergentAngle - A;

  // ── Prism geometry — fully dynamic from A ────────────────────────────
  // ViewBox: 0 0 600 500
  // Apex fixed at (300, 100), prism height = 280px.
  // Half-base = prismH / tan(baseAngle), baseAngle = (180-A)/2
  const cx       = 300;
  const apexY    = 100;
  const prismH   = 280;

  const baseAngleDeg = (180 - A) / 2;
  const baseAngleRad = (baseAngleDeg * Math.PI) / 180;
  // Guard against degenerate A (0 or 180)
  const halfBase = Math.abs(Math.tan(baseAngleRad)) > 0.001
    ? prismH / Math.tan(baseAngleRad)
    : 300;

  const apex  = { x: cx,            y: apexY          };
  const baseL = { x: cx - halfBase, y: apexY + prismH };
  const baseR = { x: cx + halfBase, y: apexY + prismH };

  // Left face vector & outward normal
  const lfVx   = baseL.x - apex.x;
  const lfVy   = baseL.y - apex.y;
  const lfLen  = Math.sqrt(lfVx * lfVx + lfVy * lfVy);
  // Outward normal = 90° CCW of face direction
  const leftNormX        = -lfVy / lfLen;
  const leftNormY        =  lfVx / lfLen;
  const leftNormAngleDeg = (Math.atan2(leftNormY, leftNormX) * 180) / Math.PI;

  // Entry point on left face (28% from apex)
  const t  = 0.28;
  const P1 = { x: apex.x + t * lfVx, y: apex.y + t * lfVy };

  // Incident ray
  const inwardNormAngle     = leftNormAngleDeg + 180;
  const incidentDirAngleDeg = inwardNormAngle - incidentAngle;
  const incidentDirRad      = (incidentDirAngleDeg * Math.PI) / 180;
  const laserDist = 110;
  const P0 = {
    x: P1.x - laserDist * Math.cos(incidentDirRad),
    y: P1.y - laserDist * Math.sin(incidentDirRad),
  };

  // Internal ray direction
  const insideDirAngleDeg = inwardNormAngle + (r1_rad * 180) / Math.PI;
  const insideDirRad      = (insideDirAngleDeg * Math.PI) / 180;

  // P2: intersection of internal ray with right face
  const rfVx = baseR.x - apex.x;
  const rfVy = baseR.y - apex.y;
  const rdx  = Math.cos(insideDirRad);
  const rdy  = Math.sin(insideDirRad);
  const det  = rdx * (-rfVy) - (-rfVx) * rdy;
  let s_param = 0.5;
  if (Math.abs(det) > 1e-10) {
    const s_raw = (rdx * (apex.y - P1.y) - (apex.x - P1.x) * rdy) / det;
    s_param = Math.min(Math.max(s_raw, 0.02), 0.98);
  }
  const P2 = { x: apex.x + s_param * rfVx, y: apex.y + s_param * rfVy };

  // Right face outward normal (90° CW of face direction)
  const rfLen             = Math.sqrt(rfVx * rfVx + rfVy * rfVy);
  const rightNormX        =  rfVy / rfLen;
  const rightNormY        = -rfVx / rfLen;
  const rightNormAngleDeg = (Math.atan2(rightNormY, rightNormX) * 180) / Math.PI;

  // Emergent ray
  const emergentDirAngleDeg = rightNormAngleDeg - emergentAngle;
  const emergentDirRad      = (emergentDirAngleDeg * Math.PI) / 180;
  const emergentLen = 150;
  const maxY = apexY + prismH + 95;
  const P3raw = {
    x: P2.x + emergentLen * Math.cos(emergentDirRad),
    y: P2.y + emergentLen * Math.sin(emergentDirRad),
  };
  const P3 = P3raw.y > maxY
    ? { x: P2.x + (maxY - P2.y) / (P3raw.y - P2.y) * (P3raw.x - P2.x), y: maxY }
    : P3raw;

  // ── GSAP refs ─────────────────────────────────────────────────────────
  const incidentRayRef = useRef<SVGLineElement>(null);
  const insideRayRef   = useRef<SVGLineElement>(null);
  const emergentRayRef = useRef<SVGLineElement>(null);
  const spectrumRef    = useRef<SVGGElement>(null);
  const normal1Ref     = useRef<SVGLineElement>(null);
  const normal2Ref     = useRef<SVGLineElement>(null);

  useEffect(() => {
    const normalLen = 65;

    gsap.set(normal1Ref.current, {
      attr: {
        x1: P1.x + normalLen * leftNormX,  y1: P1.y + normalLen * leftNormY,
        x2: P1.x - normalLen * leftNormX,  y2: P1.y - normalLen * leftNormY,
      },
    });
    gsap.set(normal2Ref.current, {
      attr: {
        x1: P2.x + normalLen * rightNormX, y1: P2.y + normalLen * rightNormY,
        x2: P2.x - normalLen * rightNormX, y2: P2.y - normalLen * rightNormY,
      },
    });

    gsap.to(incidentRayRef.current, { attr: { x1: P0.x, y1: P0.y, x2: P1.x, y2: P1.y }, duration: 0.25 });
    gsap.to(insideRayRef.current,   { attr: { x1: P1.x, y1: P1.y, x2: P2.x, y2: P2.y }, duration: 0.25 });

    if (TIR) {
      const reflectAngleRad =
        (insideDirAngleDeg - 2 * (insideDirAngleDeg - (rightNormAngleDeg + 180))) * Math.PI / 180;
      gsap.to(emergentRayRef.current, {
        attr: {
          x1: P2.x, y1: P2.y,
          x2: P2.x + 100 * Math.cos(reflectAngleRad),
          y2: P2.y + 100 * Math.sin(reflectAngleRad),
        },
        duration: 0.25,
      });
      gsap.to(spectrumRef.current, { opacity: 0, duration: 0.25 });
    } else {
      gsap.to(emergentRayRef.current, { attr: { x1: P2.x, y1: P2.y, x2: P3.x, y2: P3.y }, duration: 0.25 });
      gsap.to(spectrumRef.current, { opacity: 1, duration: 0.35 });
    }
  }, [
    A, incidentAngle, materialIdx, TIR,
    P0.x, P0.y, P1.x, P1.y, P2.x, P2.y, P3.x, P3.y,
    leftNormX, leftNormY, rightNormX, rightNormY,
    insideDirAngleDeg, rightNormAngleDeg,
  ]);

  // ── Spectrum fan ──────────────────────────────────────────────────────
  const spectrumRay = (deltaEDeg: number) => {
    const dir = ((emergentDirAngleDeg + deltaEDeg) * Math.PI) / 180;
    const rx = P2.x + emergentLen * Math.cos(dir);
    const ry = P2.y + emergentLen * Math.sin(dir);
    if (ry > maxY) {
      return { x2: P2.x + (maxY - P2.y) / (ry - P2.y) * (rx - P2.x), y2: maxY };
    }
    return { x2: rx, y2: ry };
  };

  const specColors = [
    { color: 'rgba(255,0,0,0.9)',   delta: -3.0 },
    { color: 'rgba(255,140,0,0.9)', delta: -1.8 },
    { color: 'rgba(255,255,0,0.9)', delta: -0.8 },
    { color: 'rgba(0,200,0,0.9)',   delta:  0.2 },
    { color: 'rgba(0,80,255,0.9)',  delta:  1.4 },
    { color: 'rgba(180,0,255,0.9)', delta:  2.8 },
  ];

  // ── Record observation ────────────────────────────────────────────────
  const recordObservation = () => {
    if (!hasAdjustedSlider['prism-refraction']) {
      setValidationError(
        'Setup Incomplete',
        'You cannot record data before adjusting the apparatus.',
        'Adjust the incident angle, prism angle, or material first.'
      );
      return;
    }
    if (incidentAngle < 30) {
      setValidationError(
        'Angle Too Narrow',
        "At an incidence angle below 30°, deviation curve characteristics won't be easily observable.",
        'Set the incident angle to at least 30°.'
      );
      return;
    }
    if (TIR) {
      setValidationError(
        'Total Internal Reflection',
        'Light cannot exit the prism at this angle/material combination.',
        'Reduce the incident angle or choose a material with a lower refractive index.'
      );
      return;
    }
    addObservation({
      'Angle i (°)':  incidentAngle,
      'Prism A (°)':  A,
      'Angle r₁ (°)': Number(((r1_rad * 180) / Math.PI).toFixed(2)),
      'Angle e (°)':  Number(emergentAngle.toFixed(2)),
      'Angle δ (°)':  Number(deviation.toFixed(2)),
    });
  };

  // ── Deviation arc ─────────────────────────────────────────────────────
  const deviationArc = !TIR ? (() => {
    const arcR       = 40;
    const startAngle = incidentDirRad + Math.PI;
    const endAngle   = emergentDirRad;
    const sx = P1.x + arcR * Math.cos(startAngle);
    const sy = P1.y + arcR * Math.sin(startAngle);
    const ex = P1.x + arcR * Math.cos(endAngle);
    const ey = P1.y + arcR * Math.sin(endAngle);
    return (
      <>
        <path
          d={`M ${sx} ${sy} A ${arcR} ${arcR} 0 0 1 ${ex} ${ey}`}
          fill="none" stroke="yellow" strokeWidth="1" strokeDasharray="3,3"
        />
        <text x={P1.x - 30} y={P1.y + 12} fill="yellow" fontSize="13" fontStyle="italic">δ</text>
      </>
    );
  })() : null;

  // ── Shared SVG element ────────────────────────────────────────────────
  const svgDiagram = (
    <svg
      width="100%"
      height="100%"
      viewBox="0 0 600 500"
      preserveAspectRatio="xMidYMid meet"
      style={{ overflow: 'visible', display: 'block' }}
    >
      {/* Prism shape — updates live with A slider */}
      <polygon
        points={`${apex.x},${apex.y} ${baseL.x},${baseL.y} ${baseR.x},${baseR.y}`}
        fill="rgba(0,212,255,0.08)"
        stroke="rgba(0,212,255,0.55)"
        strokeWidth="2.5"
      />
      <text x={apex.x} y={apex.y - 12} textAnchor="middle" fill="rgba(0,212,255,0.7)" fontSize="12" fontWeight="bold">
        A = {A}°
      </text>

      {/* Normals */}
      <line ref={normal1Ref} stroke="#475569" strokeDasharray="5,4" strokeWidth="1" />
      <line ref={normal2Ref} stroke="#475569" strokeDasharray="5,4" strokeWidth="1" />

      {/* Rays */}
      <line ref={incidentRayRef} stroke="white"                strokeWidth="3" strokeLinecap="round" />
      <line ref={insideRayRef}   stroke="rgba(0,212,255,0.85)" strokeWidth="3" strokeLinecap="round" />
      <line ref={emergentRayRef} stroke="white"                strokeWidth="3" strokeLinecap="round" />

      {/* Spectrum + deviation arc */}
      <g ref={spectrumRef} opacity={TIR ? 0 : 1}>
        {!TIR && specColors.map(({ color, delta }) => {
          const { x2, y2 } = spectrumRay(delta);
          return (
            <line
              key={delta}
              x1={P2.x} y1={P2.y} x2={x2} y2={y2}
              stroke={color} strokeWidth="3" strokeLinecap="round"
            />
          );
        })}
        {deviationArc}
      </g>

      {TIR && (
        <text
          x={cx} y={apexY + prismH + 70}
          textAnchor="middle" fill="orange" fontSize="13" fontWeight="bold"
        >
          Total Internal Reflection — no emergent ray
        </text>
      )}

      <text x={P0.x + 10} y={P0.y - 8} fill="white" fontSize="11" opacity={0.7}>
        i = {incidentAngle}°
      </text>
    </svg>
  );

  // ── Metrics pills ─────────────────────────────────────────────────────
  const metricsPills = (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
      {[
        { label: 'A',  value: `${A}°`,                                         color: '#a78bfa' },
        { label: 'i',  value: `${incidentAngle}°`,                             color: '#00d4ff' },
        { label: 'r₁', value: `${((r1_rad * 180) / Math.PI).toFixed(1)}°`,    color: '#00d4ff' },
        { label: 'e',  value: TIR ? 'T.I.R' : `${emergentAngle.toFixed(1)}°`, color: TIR ? '#f59e0b' : '#00d4ff' },
        { label: 'δ',  value: TIR ? '—' : `${deviation.toFixed(1)}°`,         color: TIR ? '#f59e0b' : '#22c55e' },
      ].map(({ label, value, color }) => (
        <div key={label} style={{
          background: 'rgba(0,0,0,0.55)',
          border: '1px solid rgba(255,255,255,0.1)',
          borderRadius: 6,
          padding: isMobile ? '3px 7px' : '4px 10px',
          fontFamily: 'monospace',
          fontSize: isMobile ? 10 : 11,
          color,
          whiteSpace: 'nowrap',
        }}>
          <span style={{ color: '#64748b', marginRight: 3 }}>{label} =</span>{value}
        </div>
      ))}
    </div>
  );

  // ════════════════════════════════════════════════════════════════════
  // MOBILE LAYOUT
  // ════════════════════════════════════════════════════════════════════
  if (isMobile) {
    return (
      <div style={{
        width: '100%',
        height: '100%',
        display: 'flex',
        flexDirection: 'column',
        background: '#050B14',
        overflow: 'hidden',
      }}>
        {/* SVG fills all available space */}
        <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
          {svgDiagram}
        </div>

        {/* Bottom bar — metrics + record button, never overlaps SVG */}
        <div style={{
          flexShrink: 0,
          background: 'rgba(5,11,20,0.97)',
          borderTop: '1px solid rgba(255,255,255,0.07)',
          padding: '8px 12px 10px',
          display: 'flex',
          flexDirection: 'column',
          gap: 8,
        }}>
          {metricsPills}
          <button
            onClick={recordObservation}
            style={{
              width: '100%',
              padding: '10px 0',
              background: TIR ? 'rgba(100,116,139,0.12)' : 'rgba(59,130,246,0.18)',
              color:      TIR ? '#64748b'                 : '#60a5fa',
              border: `1px solid ${TIR ? 'rgba(100,116,139,0.3)' : 'rgba(59,130,246,0.5)'}`,
              borderRadius: 8,
              fontWeight: 700,
              fontSize: 13,
              cursor: 'pointer',
              transition: 'all 0.15s',
              WebkitTapHighlightColor: 'transparent',
            }}
          >
            {TIR ? 'T.I.R — Cannot Record' : 'Record δ (Deviation)'}
          </button>
        </div>
      </div>
    );
  }

  // ════════════════════════════════════════════════════════════════════
  // DESKTOP LAYOUT
  // ════════════════════════════════════════════════════════════════════
  return (
    <div className="w-full h-full flex flex-col items-center justify-center relative bg-[#050B14]">

      {/* Optical metrics — top-left */}
      <div className="absolute top-4 left-4 z-10">
        <div className="bg-black/60 border border-gray-700 px-4 py-2 rounded">
          <div className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">
            Optical Metrics
          </div>
          <div className="font-mono text-sm flex flex-col gap-1">
            <span style={{ color: '#a78bfa' }}>Prism Angle: A  = {A}°</span>
            <span className="text-accent-cyan">Incidence:   i  = {incidentAngle}°</span>
            <span className="text-accent-cyan">Refraction:  r₁ = {((r1_rad * 180) / Math.PI).toFixed(2)}°</span>
            <span className="text-accent-cyan">Emergence:   e  = {TIR ? 'T.I.R' : emergentAngle.toFixed(2) + '°'}</span>
            <span style={{ color: '#22c55e' }}>Deviation:   δ  = {TIR ? '—' : deviation.toFixed(2) + '°'}</span>
          </div>
        </div>
      </div>

      <svg
        width="100%"
        height="90%"
        viewBox="0 0 600 500"
        preserveAspectRatio="xMidYMid meet"
        className="overflow-visible"
      >
        <polygon
          points={`${apex.x},${apex.y} ${baseL.x},${baseL.y} ${baseR.x},${baseR.y}`}
          fill="rgba(0,212,255,0.08)"
          stroke="rgba(0,212,255,0.55)"
          strokeWidth="2.5"
        />
        <text x={apex.x} y={apex.y - 12} textAnchor="middle" fill="rgba(0,212,255,0.7)" fontSize="12" fontWeight="bold">
          A = {A}°
        </text>

        <line ref={normal1Ref} stroke="#475569" strokeDasharray="5,4" strokeWidth="1" />
        <line ref={normal2Ref} stroke="#475569" strokeDasharray="5,4" strokeWidth="1" />

        <line ref={incidentRayRef} stroke="white"                strokeWidth="3" strokeLinecap="round" />
        <line ref={insideRayRef}   stroke="rgba(0,212,255,0.85)" strokeWidth="3" strokeLinecap="round" />
        <line ref={emergentRayRef} stroke="white"                strokeWidth="3" strokeLinecap="round" />

        <g ref={spectrumRef} opacity={TIR ? 0 : 1}>
          {!TIR && specColors.map(({ color, delta }) => {
            const { x2, y2 } = spectrumRay(delta);
            return (
              <line
                key={delta}
                x1={P2.x} y1={P2.y} x2={x2} y2={y2}
                stroke={color} strokeWidth="3" strokeLinecap="round"
              />
            );
          })}
          {deviationArc}
        </g>

        {TIR && (
          <text
            x={cx} y={apexY + prismH + 70}
            textAnchor="middle" fill="orange" fontSize="13" fontWeight="bold"
          >
            Total Internal Reflection — no emergent ray
          </text>
        )}

        <text x={P0.x + 10} y={P0.y - 8} fill="white" fontSize="11" opacity={0.7}>
          i = {incidentAngle}°
        </text>
      </svg>

      {/* Record button — top-right */}
      <div className="absolute top-4 right-4 bg-black/80 px-4 py-3 border border-gray-700 rounded min-w-[200px] z-10">
        <button
          onClick={recordObservation}
          disabled={TIR}
          className="w-full py-2 bg-blue-500/20 text-blue-400 border border-blue-500/50 hover:bg-blue-500/30 rounded font-bold disabled:opacity-50 transition-colors"
        >
          Record δ (Deviation)
        </button>
      </div>
    </div>
  );
}